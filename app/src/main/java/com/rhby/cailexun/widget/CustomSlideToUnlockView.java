...
import com.rhby.cailexun.utils.DensityUtil;
...
//        textSize=mTypedArray.getInteger(R.styleable.SlideToUnlockView_textSizes, 11);
...
//        tv_hint.setTextSize(DensityUtil.sp2px(getContext(), textSize));
        ...
        tv_hint.setText(TextUtils.isEmpty(textHint)? "向右滑动即可登录":textHint);
...
//                        }
    ...
    private void resetTextViewAlpha(int distance) {
        ...
        if(Math.abs(distance)>=Math.abs(DISTANCE_LIMIT)){
            ...
            iv_slide.setAlpha(0.6f);
        }
        else{
            tv_hint.setAlpha(1.0f-Math.abs(distance)*1.0f/Math.abs(DISTANCE_LIMIT));
            iv_slide.setAlpha(1.0f-Math.abs(distance)*0.4f/Math.abs(DISTANCE_LIMIT));
        }
    }
    ...
    private void scrollToLeft(final View v) {
        ...
        ViewAnimator
                .animate( rl_slide)
                .translationX(ViewHelper.getTranslationX(v), -params1.leftMargin+DensityUtil.dp2px(mContext,20))
