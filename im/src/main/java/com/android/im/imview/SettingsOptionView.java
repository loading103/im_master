...
public class SettingsOptionView extends RelativeLayout {
    ...
    public SettingsOptionView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SettingsOptionView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context,attrs);
    }

    private void init(Context context,AttributeSet attrs){
        View view = LayoutInflater.from(context).inflate(R.layout.view_settings_option,this);
        initView(view);
    ...
    private void initAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.SettingsOptionView_left_img) {
            //设置图标
            setLeftImg(typedArray.getResourceId(attr,R.mipmap.im_chat_setting));
        }
