...
import com.android.im.imutils.IMFaceData;
import com.android.im.imutils.IMGifOpenHelper;
import com.android.nettylibrary.utils.IMScreenUtil;
...
public class IMGifTextView extends EditText {
    /**
     ...
     */
    @SuppressLint("NewApi")
    public IMGifTextView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        // TODO Auto-generated constructor stub
        // spanInfoList = new ArrayList<SpanInfo>();
        IMGifTextView.this.setFocusableInTouchMode(false);
    }

    @SuppressLint("NewApi")
    public IMGifTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        // TODO Auto-generated constructor stub
        // spanInfoList = new ArrayList<SpanInfo>();
        IMGifTextView.this.setFocusableInTouchMode(false);
    ...
    @SuppressLint("NewApi")
    public IMGifTextView(Context context) {
        super(context);
        // TODO Auto-generated constructor stub
        // spanInfoList = new ArrayList<SpanInfo>();
        IMGifTextView.this.setFocusableInTouchMode(false);
...
//		Pattern mPattern = Pattern.compile("\\[:..\\]|\\[:...\\]");
        ...
        while (mMatcher.find()) {
             ...
             */
            if ((faceId = IMFaceData.gifFaceInfo.get(faceName)) != null) {
                if (isGif) {
    ...
    private void parseGif(int resourceId, int start, int end) {
        ...
        IMGifOpenHelper helper = new IMGifOpenHelper();
...
////				// TODO Auto-generated method stub
    ...
    public boolean parseMessage(IMGifTextView IMGifTextView) {
        if (IMGifTextView.myText != null && !IMGifTextView.myText.equals("")) {
            SpannableString sb = new SpannableString("" + IMGifTextView.myText); // 获得要显示的文本
            int gifCount = 0;
            SpanInfo info = null;
            for (int i = 0; i < IMGifTextView.spanInfoList.size(); i++) { // for循环，处理显示多个图片的问题
                info = IMGifTextView.spanInfoList.get(i);
                 ...
                 */
                int size = IMScreenUtil.dip2px(IMGifTextView.getContext(), 30);
                if (gifCount != 0) {
                ...
                ImageSpan imageSpan = new ImageSpan(IMGifTextView.getContext(),
                        bitmap);
    ...
    public static final class TextRunnable implements Runnable {
        private final WeakReference<IMGifTextView> mWeakReference;

        public TextRunnable(IMGifTextView f) {
            mWeakReference = new WeakReference<IMGifTextView>(f);
        }

        @Override
        public void run() {
            // TODO Auto-generated method stub
            IMGifTextView IMGifTextView = mWeakReference.get();
            if (IMGifTextView != null) {
                /**
                 * 这一步是为了节省内存而是用，即如果文本中只有静态图片没有动态图片，那么该线程就此终止，不会重复执行
                 * 。而如果有动图，那么会一直执行
                 */
                if (IMGifTextView.parseMessage(IMGifTextView)) {
                    IMGifTextView.handler.postDelayed(this, DELAYED);
                }
            }
        }
    }
