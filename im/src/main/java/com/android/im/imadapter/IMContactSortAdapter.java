...
import com.android.im.imui.activity.IMTotleMemberActivity;
 ...
 */ 
public class IMContactSortAdapter extends BaseAdapter implements SectionIndexer {
	private List<IMGroupMemberBean> list = null;
	private Context mContext;
	private boolean isat;
	public IMContactSortAdapter(Context mContext, List<IMGroupMemberBean> list,boolean isat) {
		...
		holder.line2.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if(IMStopClickFast.isFastClick()) {
					if(isat){
						EventBus.getDefault().post(new IMAtSomeOneEvent(data));
						((IMTotleMemberActivity)mContext).finish();
					}else {
						Intent intent = new Intent(mContext, IMMemberInforActivity.class);
