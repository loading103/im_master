...
import com.android.nettylibrary.handler.HeartbeatHandler;
import com.android.nettylibrary.imconstant.ExecutorServiceFactory;
import com.android.nettylibrary.imconstant.MsgDispatcher;
import com.android.nettylibrary.imconstant.MsgTimeoutTimerManager;
import com.android.nettylibrary.interfac.IMSClientInterface;
import com.android.nettylibrary.listener.IMSConnectStatusCallback;
import com.android.nettylibrary.listener.OnEventListener;
...
import static com.android.nettylibrary.IMSConfig.MSG_SYSTEM_DISSCONNECT;
...
public class NettyTcpClient implements IMSClientInterface {
    ...
    private String currentHost = "";// 当前连接host
    private int currentPort = -1;// 当前连接port

    private MsgTimeoutTimerManager msgTimeoutTimerManager;// 消息发送超时定时器管理
    public static ConcurrentHashMap<String, Integer> connectMap = new ConcurrentHashMap<>();//记录是否连接成功  0未连接  1连接成功  2正在连接
     ...
     */
    @Override
    public void sendMsg(MessageProtobuf.ImMessage msg) {
     ...
     */
    @Override
    public void sendMsg(MessageProtobuf.ImMessage msg, boolean isJoinTimeoutManager) {
        if (msg == null ) {
            System.out.println("------发送消息失败，消息为空\tmessage=" + msg);
            return;
        }

        if(!StringUtil.isNullOrEmpty(msg.getFingerprint())) {
...
//                IMSNettyManager.getInstance().regetImToken(IMSConfig.MSG_SYSTEM_RECONNET);//断线重新获取ip 端口  token
            ...
            case IMSConfig.CONNECT_STATE_SUCCESSFUL: {
                String hostID = IMPreferenceUtil.getPreference_String(IMSConfig.IM_SAVE_HOST, IMSConfig.IMS_SERVICE_IP);
                System.out.println(String.format("ims连接成功，hostID="+hostID));
                Log.e("ims连接成功-----", "handshakeMsg");
                IMSNettyManager.getInstance().regetImToken(IMSConfig.MSG_SYSTEM_GETOFFLIN_MSG);
                if (mIMSConnectStatusCallback != null) {
                    mIMSConnectStatusCallback.onConnected();
                }
                // 连接成功，发送握手消息
                MessageProtobuf.ImMessage handshakeMsg = getHandshakeMsg();
...
//                IMSNettyManager.getInstance().regetImToken(IMSConfig.MSG_SYSTEM_DISSCONNECT);
    ...
    public void addHeartbeatHandler() {
        ...
        try {
            ...
            channel.pipeline().addFirst(IdleStateHandler.class.getSimpleName(), new IdleStateHandler(
                    heartbeatInterval * 3000, heartbeatInterval, 0, TimeUnit.MILLISECONDS));
    ...
    private boolean isNetworkAvailable() {
        if (mOnEventListener != null) {
            return mOnEventListener.isNetworkAvailable(IMSClientBootstrap.getInstance().getContext());
        }
    ...
    private void toServer() {
        System.err.println("hostId444。。。2。"+currentHost);
        connectMap.put(currentHost, 2);
        try {
            String hostID = IMPreferenceUtil.getPreference_String(IMSConfig.IM_SAVE_HOST, IMSConfig.IMS_SERVICE_IP);
            currentHost=hostID.substring(0,hostID.indexOf(":"));
            currentPort=Integer.parseInt(hostID.substring(hostID.indexOf(":")+1,hostID.length()));

            channel = bootstrap.connect(currentHost, currentPort).sync().channel();
            if (channel.isActive()) {
                System.err.println("hostId555。。。1。"+currentHost);
                connectMap.put(currentHost, 1);
            }
        } catch (Exception e) {
            try {
                Thread.sleep(500);
            ...
            channel = null;
            System.err.println("hostId666。。。0。"+currentHost);
            connectMap.put(currentHost, 0);
        }
    }
    ...
    private class ResetConnectRunnable implements Runnable {
        ...
        private int connectServer() {
            ...
            currentHost=hostID.substring(0,hostID.indexOf(":"));
            currentPort=Integer.parseInt(hostID.substring(hostID.indexOf(":")+1,hostID.length()));
            String IP= currentHost+" "+currentPort;
            serverUrlList.clear();
            serverUrlList.add(IP);
            ...
            for (int i = 0; (!isClosed && i < serverUrlList.size()); i++) {
                ...
                for (int j = 1; j <= IMSConfig.DEFAULT_RECONNECT_COUNT; j++) {
                    ...
                    try {
                        ...
                        boolean a = !connectMap.containsKey(currentHost);

                        if (!connectMap.containsKey(currentHost) || connectMap.get(currentHost) == 0) {
